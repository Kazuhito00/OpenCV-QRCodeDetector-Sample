...
def main():
    ...
    while True:
        ...
        # 検出実施 #############################################################
        ret, *results = qrcode_detector.detectAndDecodeMulti(image)

        # 描画 ################################################################
        debug_image = draw_tags(debug_image, results, elapsed_time)
...
def draw_tags(
    ...
    qrcode_texts = qrcode_results[0]
    qrcode_corners = qrcode_results[1]
    # qrcode_straights = qrcode_results[2]

    if len(qrcode_texts) > 0:
        for text, corner in zip(qrcode_texts, qrcode_corners):
